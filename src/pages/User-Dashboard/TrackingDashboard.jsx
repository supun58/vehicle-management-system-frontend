...
const TrackingDashboard = () => {
  ...
  // Temporary storage for initial WebSocket data
    ...
    ws.onmessage = (e) => {
      ...
      try {
        ...
        if (data.type === 'location_update') {
          // Store initial data if we don't have it yet
          if (!initialDataRef.current.taskId && data.taskId) {
          ...
          // Update with new data if taskId matches
  ...
  // Apply initial data when component mounts
  useEffect(() => {
    if (initialDataRef.current.taskId) {
      setTaskId(initialDataRef.current.taskId);
      if (initialDataRef.current.location) {
        setDriverLocation(initialDataRef.current.location);
      }
      fetchTaskDetails(initialDataRef.current.taskId);
    }
  }, []);
  ...
  }, [isLoaded, task, calculateRoute, hasAccess]);
